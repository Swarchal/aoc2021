...
type
  BitArray = array[12, range[0..1]]


func asBitArray(l: string): BitArray =
  for i, ch in l:
    result[i] = parseInt($ch)


func asDecimal(b: BitArray): int =
  return b.join.parseBinInt


proc parseInput(file="input.txt"): seq[BitArray] =
  return file.readFile.splitLines.map(asBitArray)


func mostCommon(b: seq[BitArray]): BitArray =
  var count: array[12, int]
  let n = b.len
  for i in b:
    for idx, j in i:
      count[idx] += j
  for idx, i in count:
    result[idx] = (i / n).round.toInt


func leastCommon(mostCommon: BitArray): BitArray =
  for idx, i in mostCommon:
    if i == 0:
      result[idx] = 1
    else:
      result[idx] = 0
...
when isMainModule:
  let
    input = parseInput()
    gamma = input.mostCommon()
    epsilon = gamma.leastCommon()
  echo gamma.asDecimal * epsilon.asDecimal
