...
proc simulateFish(age: int, total_days: int): int {.memoized.} =
    if total_days == 0:
        # end of tiiime, return this fish
        return 1
    if age == 0:
        # age is zero, crate new fish and recycle age
        return simulateFish(6, total_days-1) + simulateFish(8, total_days-1)
    else:
        # nothing happens, age fish 1 day
        return simulateFish(age-1, total_days-1)


proc simulate(input: seq[int], days: int): int =
    var fishCount = 0
    for fish in input:
        fishCount += simulateFish(fish, days)
    return fishCount


let fishes = parseInput()

echo fishes.simulate(days=80)
