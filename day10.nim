...
proc parseInput(file="input.txt"): seq[string] =
    return file.readFile().strip().splitLines()
...
proc score(line: string): int =
    ...
    for c in line:
        if c in opening:
            stack.addLast(c)
        if c in closing:
            if matching[c] != stack.popLast():
                return errorScore[c]


proc part1() =
    var total = 0
    for line in parseInput("input.txt"):
        total += line.score()
    echo total
...
proc completeScore(line: string): int =
    var stack = initDeque[char]()
    for c in line:
        if c in opening:
            stack.addLast(c)
    ...
    return stack.toSeq.score()


func median(s: seq[int]): int =
    return sorted(s, Ascending)[s.len().div(2)]
...
proc part2() =
    ...
    for line in parseInput():
        if line.score() == 0:
            # is incomplete, not corrupted
            var score = line.completeScore()
            scores.add(score)
    echo median(scores)
